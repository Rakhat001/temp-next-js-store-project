...
const ProductsContainer = async ({ layout, search }: { layout: string, search: string }) => {
  ...
  return (
    <>
      ...
      {/* Products */}
      <div>
        {totalProducts === 0 ? <h5 className='text-2xl mt-16'>Sorry no products matched in your search...</h5> : layout === 'grid' ? <ProductsGrid products={products} /> : <ProductsList products={products} />}
      </div>
    </>
  )
}
